defmodule ElNino.Song.Supervisor do
  ...
  def pair_exists?(guild_id) do
    with [{_manager_pid, _value}] <- Registry.lookup(GuildSongManagerRegistry, ElNino.Common.via_guild_manager_registry(guild_id)),
         [{_queue_pid, _value}] <- Registry.lookup(GuildSongQueueRegistry, ElNino.Common.via_guild_queue_registry(guild_id)) do
      true
    else
      _ -> false
    end
